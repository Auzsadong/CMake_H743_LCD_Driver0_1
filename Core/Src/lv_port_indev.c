...
#include "lvgl.h"
#include "ft6336u.h" // 替换原有的 touch.h
/*********************
 *      DEFINES
 *********************/
...
{
    // 1. 底层硬件初始化
    // (如果你已经在 main.c 里调用过类似 Touch_Init() 的函数，这里可以留空)

    // 2. 创建并注册触摸屏输入设备
    lv_indev_t * indev_touchpad = lv_indev_create();
    lv_indev_set_type(indev_touchpad, LV_INDEV_TYPE_POINTER); // 设置为指针类型
    lv_indev_set_read_cb(indev_touchpad, touchpad_read);      // 绑定读取回调函数
}
...
{
    static FT6336U_Touch_t touch_data;

    // 1. 检查 TP_INT (PC1) 中断引脚
    // FT6336U 按下时 INT 为低电平
    if(HAL_GPIO_ReadPin(TP_INT_GPIO_Port, TP_INT_Pin) == GPIO_PIN_RESET) {

        // 2. 调用我们之前写好的底层驱动
        FT6336U_Get_Touch(&touch_data);

        if(touch_data.is_pressed) {
            data->state = LV_INDEV_STATE_PRESSED;

            /* 应用你发现的坐标翻转逻辑 */
            data->point.x = 319 - touch_data.x;
            data->point.y = 479 - touch_data.y;
